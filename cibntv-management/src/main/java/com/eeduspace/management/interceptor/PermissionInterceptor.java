...
public class PermissionInterceptor extends HandlerInterceptorAdapter{
	...
	public boolean preHandle(HttpServletRequest request,HttpServletResponse response, Object handler) throws Exception {
		...
		if (StringUtils.isEmpty(session.getAttribute("uuid")) || StringUtils.isEmpty(session.getAttribute("roleModel"))) {
			ri.setData(LOGIN_URL);
			response.getWriter().print(gson.toJson(ri));
			return false;
		}
//		SessionItem si = (SessionItem) obj;
//		System.out.println("session里的值：" + gson.toJson(si));
		String flag = "disable";
		...
		if (!StringUtils.isEmpty(roleModel) && roleModel.getStatus().equals(String.valueOf(RoleEnum.Status.Enable))) {
			...
			if (!StringUtils.isEmpty(pModels) && !pModels.isEmpty()) {
		...
		System.out.println("权限判断结果：" + flag);
